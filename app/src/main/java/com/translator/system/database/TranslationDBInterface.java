...
public class TranslationDBInterface extends ADBWorker {
    ...
    public Cursor getHistory() {
        String selectQuery = "SELECT * FROM " + TRANSLATION_TABLE_NAME + " WHERE " + TRANSLATION_COLUMN_IN_HISTORY + " =?";
        return getCursor(selectQuery, new String[]{ "1" });
    }


    public Cursor getFavorite() {
        String selectQuery = "SELECT * FROM " + TRANSLATION_TABLE_NAME + " WHERE " + TRANSLATION_COLUMN_IN_FAVORITE + " =?";
        return getCursor(selectQuery, new String[]{ "1" });
    }
